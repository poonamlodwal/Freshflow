...
schemas.py
----------
Pydantic v2 schemas for Phase 6 Demo Polish API.
"""
...
class CorrectSampleRequest(BaseModel):
    correctedLabel: str = Field(..., description="Correct label provided by user.")


class RetrainRequest(BaseModel):
    sampleLimit: Optional[int] = Field(100, ge=1, description="Max training samples for retrain batch.")
...
class DemoSeedResponse(BaseModel):
    ...
    claimsCount: int
    eventsCount: int
    retrainHistoryCount: int


class NarrativeStepResponse(BaseModel):
    stepNumber: int
    title: str
    action: str
    description: str
...
class NarrativeScriptResponse(BaseModel):
    title: str = "FreshChain End-to-End Judge Walkthrough Script"
    totalSteps: int = 6
    estimatedDurationMinutes: float = 3.5
    steps: List[NarrativeStepResponse]


class ImpactStatementResponse(BaseModel):
    impactStatement: str = Field(..., description="2-3 line pitch impact statement")
    wasteSavedKg: float
    revenueSavedDollars: float
    modelAttribution: str
    fineTuningAccuracyImprovement: str
