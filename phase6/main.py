...
main.py
-------
FastAPI entry point for Phase 6 — Demo Polish ML & Application Service.

Routes:
  Phase 6 Demo Polish:
    POST /demo/seed                    → re-seed realistic demo data safety net
    GET  /demo/narrative               → fetch 6-step judge-ready walkthrough script
    POST /demo/narrative/run-step/{id} → run programmatic demonstration step
    GET  /demo/impact                  → pitch impact statement (waste saved, model credit)
  Ops:
    GET  /health                       → service health & model liveness
  Predict:
    POST /predict/url                  → predict & log TrainingSample
    POST /predict/upload               → predict from upload & log TrainingSample
  Phase 5 Fine-Tuning:
    GET  /samples                      → list training samples
    POST /samples/{id}/correct         → user correction for wrong prediction
    POST /retrain                      → trigger non-blocking fine-tuning
    GET  /retrain/history              → retrain history log
    GET  /admin/retrain-summary        → Admin dashboard deliverable
  Domain & ERP:
    /users, /batches, /listings, /claims, /erp/stats, /erp/inventory, /erp/suggestions, /erp/analytics
"""
...
from services import (
    ...
    get_narrative_script_service,
    get_pitch_impact_statement_service,
    get_retrain_history_list,
    list_training_samples,
    log_training_sample,
    request_claim_service,
    respond_claim_service,
    run_narrative_step_service,
)
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    logger.info("=== FreshChain Phase 6 Demo Service starting up ===")
    await init_db()
...
app = FastAPI(
    title="FreshChain Phase 6 — Demo Polish API",
    description="Clean, judge-ready end-to-end demo service with auto-seeded data safety net and pitch impact statement.",
    version="6.0.0",
    lifespan=lifespan,
)
...
# ── Phase 6 Demo Polish Endpoints ─────────────────────────────────────────────

@app.post("/demo/seed", response_model=DemoSeedResponse, tags=["Phase 6 Demo Polish"])
async def trigger_demo_seed(db: AsyncSession = Depends(get_session)):
    """Re-seed realistic demo data safety net."""
    return await seed_demo_data(db)


@app.get("/demo/narrative", response_model=NarrativeScriptResponse, tags=["Phase 6 Demo Polish"])
async def get_demo_narrative():
    """Fetch 6-step judge-ready walkthrough script."""
    return get_narrative_script_service()


@app.post("/demo/narrative/run-step/{step_number}", tags=["Phase 6 Demo Polish"])
async def run_demo_narrative_step(
    step_number: int = Path(..., ge=1, le=6),
    db: AsyncSession = Depends(get_session),
):
    """Programmatically execute a step of the walkthrough narrative."""
    return await run_narrative_step_service(db, step_number)


@app.get("/demo/impact", response_model=ImpactStatementResponse, tags=["Phase 6 Demo Polish"])
async def get_demo_impact_statement(db: AsyncSession = Depends(get_session)):
    """Deliver 2-3 line pitch impact statement for presentation."""
    return await get_pitch_impact_statement_service(db)
